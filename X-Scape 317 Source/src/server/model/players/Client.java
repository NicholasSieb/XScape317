...
import server.model.players.QuestHandler;
...
public class Client extends Player {
	...
	public int amountOfItemsInBank() {
		int freeS = 0;
...
getPA().sendFrame126("  X-Scape World Top Skillers", 6399);
		for(int i = 0; i < 10; i++) {
			if(ranks[i] > 0) {
    ...
    public static String[] rankPpl = new String[11];
		...
		for(int iD = 29166; iD <= 29264;iD++){
			getPA().sendFrame126("", iD);
		}
		getPA().sendFrame126("Quest Points: "+QH().tqp, 29155);
		if (QH().q1 == 0) {
			getPA().sendFrame126(""+QH().q1Name, 29165);//q1
		}
		if (QH().q1 > 0) {
			getPA().sendFrame126("@yel@"+QH().q1Name, 29165);//q10
		}
		if (QH().q1 == 15) {
			getPA().sendFrame126("@gre@"+QH().q1Name, 29165);//q10
		}
	}
	public void clearQuestInterface() {
		...
		getPA().sendFrame126("Combat Level: "+getCombatLevel()+"", 19999);
	}
...
12220, 12221, 12222, 12223 };
	...
	public void initialize() {
		synchronized (this) {
			...
			setSidebarInterface(13, 29500);
			setSidebarInterface(0, 2423);
			clearQInterface();
			clearQuestInterface();
			sendMessage("<col=12595455>Welcome <col=65535> to <col=16711680>  X-Scape <col=16711784> 317.");
			...
			sendMessage("<col=4649520>Idle <col=255> #xscape <col=16711832>on <col=5446936> SwiftIRC <col=5473279> and <col=5501464> register <col=16749080> on our site, <col=65535>xscapeserver.tk.");
			sendMessage("<col=16777026>Use <col=16777215>::commands ::rules <col=16720962>look at teleports <col=255>::switch #<col=16751616> to change spellbooks.");
			sendMessage("<col=16711680>EXP Lock Status: "+expLock+"");
...
getPA().sendFrame126("X-Scape", 640);
